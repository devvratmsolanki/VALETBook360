 ...
 * envelope fields ({@code code}, {@code userMessage}). Crucially: we never leak
 * stack traces or raw SQL/driver messages to the client. Unexpected errors are
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleService(ServiceException ex, HttpServletRequest req) {
        ResponseEntity<ProblemDetail> resp = build(ex.getStatus(), ex.getCode(),
                ex.getUserMessage(), null, req);
        if (ex.getRetryAfterSeconds() > 0) {
            // RFC 7231 Retry-After (seconds). Lets a well-behaved client back off.
    ...
    public ResponseEntity<ProblemDetail> handleUnreadable(HttpMessageNotReadableException ex,
        ...
        return build(HttpStatus.BAD_REQUEST, "MALFORMED_REQUEST",
                "The request body was malformed.", null, req);
    }
    ...
    public ResponseEntity<ProblemDetail> handleIntegrity(DataIntegrityViolationException ex,
                                                         HttpServletRequest req) {
        // Most commonly the unique email index (legacy Postgres 23505).
        log.warn("Data integrity violation: {}", ex.getMostSpecificCause().getMessage());
        return build(HttpStatus.CONFLICT, "CONFLICT",
                "That record already exists.", null, req);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ProblemDetail> handleUnexpected(Exception ex, HttpServletRequest req) {
        // Log the real cause; return nothing sensitive.
