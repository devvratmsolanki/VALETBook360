...
const Users = () => {
    ...
    const fetchUsers = async () => { try { const d = await getUsers(); setUsers(d); setFiltered(d); } catch { toast.error('Failed to load'); } finally { setLoading(false); } };
    ...
    return (
        <div className="animate-fade-in">
            ...
            {loading ? <div className="flex justify-center py-12"><LoadingSpinner size="md" /></div> : filtered.length === 0 ? <Card className="p-8 text-center"><UsersIcon className="h-8 w-8 text-gray-600 mx-auto mb-3" /><p className="text-gray-500">No users found</p></Card> : (
