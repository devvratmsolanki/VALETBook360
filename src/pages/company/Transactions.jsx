    ...
    return (
        <div className="animate-fade-in">
            ...
            <Card className="overflow-hidden">{loading ? <div className="p-8 flex justify-center"><div className="animate-spin h-6 w-6 border-2 border-brand-500 border-t-transparent rounded-full" /></div> : filtered.length === 0 ? <div className="p-8 text-center text-gray-500 text-sm">No transactions found</div> : (
                <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-dark-700/50 text-xs uppercase text-gray-500"><tr><th className="px-6 py-3">Car Number</th><th className="px-6 py-3">Guest</th><th className="px-6 py-3">Phone</th><th className="px-6 py-3">Status</th><th className="px-6 py-3">Slot</th><th className="px-6 py-3">Date/Time</th><th className="px-6 py-3">Actions</th></tr></thead><tbody className="divide-y divide-white/5">{filtered.map((tx) => (<tr key={tx.id} className="hover:bg-white/[0.02] transition-colors"><td className="px-6 py-4 font-medium text-white">{tx.cars?.car_number || 'N/A'}</td><td className="px-6 py-4 text-gray-300">{tx.visitors?.name || 'Unknown'}</td><td className="px-6 py-4 text-gray-500">{tx.visitors?.phone || 'N/A'}</td><td className="px-6 py-4"><Badge variant={tx.status}>{tx.status}</Badge></td><td className="px-6 py-4 text-gray-500">{tx.parking_slot || '-'}</td><td className="px-6 py-4 text-gray-500 text-xs">{formatDateTime(tx.created_at)}</td><td className="px-6 py-4">{tx.status === 'parked' && <Button size="sm" variant="outline" onClick={() => handleStatusUpdate(tx.id, 'requested')}>Request</Button>}{tx.status === 'requested' && <Button size="sm" variant="outline" className="border-emerald-500/30 text-emerald-400" onClick={() => handleStatusUpdate(tx.id, 'ready')}>Ready</Button>}{tx.status === 'ready' && <Button size="sm" variant="ghost" onClick={() => handleStatusUpdate(tx.id, 'delivered')}>Delivered</Button>}{tx.status === 'delivered' && <span className="text-xs text-gray-600">Completed</span>}</td></tr>))}</tbody></table></div>
            )}</Card>
        </div>
    );
};
